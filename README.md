...
### 1. Preparing your Assembly File

Create a text file (e.g., `program.asm`). Ensure your file follows the Morris Mano conventions:

- **Labels** must end with a comma (e.g., `START,`).
- **Directives** like `ORG`, `DEC`, `HEX`, and `END` should be used to manage memory.
- **Comments** can be added using a semicolon `;` or `/` depending on your implementation.

### 2. Running the Assembler

Once installed, run the assembler by providing the path to your source file:
